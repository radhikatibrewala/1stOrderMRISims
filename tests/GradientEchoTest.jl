function GradientEchoTest(filepaths)
    ...
    input_params["sequence_name"]  = "gradient_echo"
    ...
    @test size(mr_signal_measurements, 2) == 29540
    count = 0
    ...
    @test abs(mr_signal_measurements[1,2]) ≈ 2.2340e-8 rtol = 1e-04
    @test abs(mr_signal_measurements[1,8]) ≈ 3.7354e-10 rtol = 1e-04
    @test abs(mr_signal_measurements[1,10]) ≈ 6.8419e-10 rtol = 1e-04
end